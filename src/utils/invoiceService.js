// src/utils/invoiceService.js
// AR invoice creation, payment recording, and write-off logic for v0.3.

import { db } from '../firebase';
import { collection, addDoc, updateDoc, doc, serverTimestamp, arrayUnion } from 'firebase/firestore';
import { generateDisplayId } from './idGenerator';

// ---------------------------------------------------------------------------
// Status helpers
// ---------------------------------------------------------------------------

/** Derive stored status from financials. Does NOT include 'overdue' — that is display-only. */
export const calcInvoiceStatus = (total, amountPaid) => {
  if (amountPaid >= total) return 'paid';
  if (amountPaid > 0) return 'partial';
  return 'unpaid';
};

/** Display-only: is the invoice overdue? (balance > 0 && dueDate < now) */
export const isOverdue = (invoice) => {
  if (!invoice || invoice.status === 'paid' || invoice.status === 'written_off') return false;
  if (!invoice.dueDate) return false;
  const due = invoice.dueDate?.toDate ? invoice.dueDate.toDate() : new Date(invoice.dueDate);
  return due < new Date();
};

/** Effective display status — overlays 'overdue' on top of stored status. */
export const displayStatus = (invoice) => {
  if (isOverdue(invoice) && invoice.status !== 'paid' && invoice.status !== 'written_off') return 'overdue';
  return invoice.status;
};

// ---------------------------------------------------------------------------
// Create invoice
// ---------------------------------------------------------------------------

/**
 * Creates a new invoice document from a completed order.
 * Call this after the order is written (Charge payment method).
 *
 * @param {Object} order  - The order object (must have items, total, customer fields)
 * @param {Object} meta   - { staffEmail, shiftId, dueDate: Date|null }
 * @returns {Promise<string>} The new invoice document ID
 */
export const createInvoice = async (order, { staffEmail, shiftId, dueDate }) => {
  const invoiceNumber = await generateDisplayId('invoices', 'INV');

  const items = (order.items || []).map(item => ({
    description: item.serviceName || item.name || 'Item',
    quantity: Number(item.quantity) || 1,
    price: Number(item.price) || 0,
    total: Number(item.price) * Number(item.quantity),
  }));

  const total = Number(order.total) || items.reduce((s, i) => s + i.total, 0);

  const invoiceDoc = {
    invoiceNumber,
    orderId: order.id || null,
    orderNumber: order.orderNumber || null,

    customerId: order.customerId || order.customer?.id || null,
    customerName: order.customerName || order.customer?.fullName || '',
    customerAddress: order.customerAddress || order.customer?.address || '',
    customerTin: order.customerTin || order.customer?.tin || '',

    items,
    subtotal: total,
    total,
    amountPaid: 0,
    balance: total,

    status: 'unpaid',
    dueDate: dueDate instanceof Date ? dueDate : (dueDate ? new Date(dueDate) : null),

    notes: '',
    createdAt: serverTimestamp(),
    staffEmail,
    shiftId,

    payments: [],
  };

  const ref = await addDoc(collection(db, 'invoices'), invoiceDoc);
  return ref.id;
};

// ---------------------------------------------------------------------------
// Record payment
// ---------------------------------------------------------------------------

/**
 * Appends a payment to an invoice and recalculates balance/status.
 * If shiftId is provided, also logs an "AR Payment" to the transactions collection.
 *
 * @param {string} invoiceId
 * @param {Object} payment   - { amount, method: 'cash'|'gcash', note, staffEmail, shiftId }
 * @param {Object} current   - Current invoice snapshot (for amountPaid, total, customer details)
 */
export const recordPayment = async (invoiceId, { amount, method, note = '', staffEmail, shiftId }, current) => {
  const entry = {
    paymentId: crypto.randomUUID(),
    amount: Number(amount),
    method,
    date: new Date(),
    staffEmail,
    note,
  };

  const newAmountPaid = Number(current.amountPaid || 0) + Number(amount);
  const newBalance = Math.max(0, Number(current.total) - newAmountPaid);
  const newStatus = calcInvoiceStatus(current.total, newAmountPaid);

  // 1. Update the Invoice Document
  await updateDoc(doc(db, 'invoices', invoiceId), {
    payments: arrayUnion(entry),
    amountPaid: newAmountPaid,
    balance: newBalance,
    status: newStatus,
  });

  // 2. Log to shift transactions (if shift is active)
  if (shiftId) {
    const tx = {
      price: Number(amount),
      quantity: 1,
      total: Number(amount),
      item: 'AR Payment',  // Special flag read by shiftFinancials.js
      notes: `Payment for Invoice #${current.invoiceNumber || invoiceId.slice(-6).toUpperCase()}${note ? ` - ${note}` : ''}`,
      paymentMethod: method === 'gcash' ? 'GCash' : 'Cash',
      customerId: current.customerId || null,
      customerName: current.customerName || 'Walk-in',
      staffEmail,
      shiftId,
      timestamp: serverTimestamp(),
      isDeleted: false
    };
    await addDoc(collection(db, 'transactions'), tx);
  }
};

// ---------------------------------------------------------------------------
// Write-off
// ---------------------------------------------------------------------------

/**
 * Writes off the remaining balance of an invoice (bad debt).
 * Requires a reason. Excluded from outstanding totals after this.
 *
 * @param {string} invoiceId
 * @param {Object} opts     - { reason, staffEmail }
 * @param {Object} current  - Current invoice snapshot
 */
export const writeOffInvoice = async (invoiceId, { reason, staffEmail }, current) => {
  const entry = {
    paymentId: crypto.randomUUID(),
    amount: Number(current.balance),
    method: 'write_off',
    date: new Date(),
    staffEmail,
    note: reason,
  };

  await updateDoc(doc(db, 'invoices', invoiceId), {
    payments: arrayUnion(entry),
    amountPaid: Number(current.total),
    balance: 0,
    status: 'written_off',
  });
};
