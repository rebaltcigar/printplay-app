...
export default function CheckoutDialog({ open, onClose, total, onConfirm, customer, defaultDueDays = 7, appSettings, isLoading = false }) {
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm" onKeyDown={handleKeyDown}>
            ...
            <DialogActions sx={{ p: 2, justifyContent: 'space-between' }}>
                ...
                <Box>
                    <Button onClick={() => handleConfirm(true)} size="large" sx={{ mr: 1 }} disabled={!canConfirm || isLoading}>
                        Print & Confirm
                    </Button>
                    <Button onClick={() => handleConfirm(false)} variant="contained" size="large" disabled={!canConfirm || isLoading}>
                        CONFIRM
                    </Button>
                </Box>
            </DialogActions>
