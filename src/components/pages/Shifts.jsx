...
import ShiftConsolidationDialog from "../dialogs/ShiftConsolidationDialog";
...
/* -------------------- component -------------------- */
const Shifts = ({ isActive = true, user }) => {
  ...
  useEffect(() => {
    const fetchCurrentShift = async () => {
      ...
      if (data) {
        setCurrentShift({ 
          id: data.id, 
          activeShiftId: data.active_shift_id, 
    ...
    const channel = supabase.channel('current-shift-shifts')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'app_status', filter: 'id=eq.current_shift' }, (payload) => {
        if (payload.new && Object.keys(payload.new).length > 0) {
          setCurrentShift({ 
            id: payload.new.id, 
            activeShiftId: payload.new.active_shift_id, 
