...
export default function ExpenseManagement({ user }) {
  ...
  const staffMap = useMemo(() => Object.fromEntries(staffOptions.map(s => [s.id, s])), [staffOptions]);

  const tableRows = useMemo(() => {
    return expenses.map((e) => {
      const tsDate = new Date(e.timestamp);
      const staff = staffMap[e.expenseStaffId];
      return {
        ...e,
        _dateOnly: toDateOnlyString(tsDate),
        _dateTime: toDateTimeString(tsDate),
        qty: Number(e.quantity || 0),
        price: Number(e.total || e.amount || 0) / Math.max(1, Number(e.quantity || 1)),
        total: Number(e.total || e.amount || 0),
        expenseStaffName: staff?.fullName || null,
      };
    });
  }, [expenses, staffMap]);
  ...
  const handleAddExpense = async (e) => {
    ...
    try {
      ...
      const expenseId = await generateDisplayId("expenses", "EX");
      ...
      const expenseDoc = {
        ...
        notes: formNotes || "",
        shift_id: null,
        financial_category: financialCategory || "OPEX",
        timestamp: transactionDate.toISOString(),
        is_deleted: false,
      };

      // 3. Add the complete document
      const { error: insertError } = await supabase.from("expenses").insert(expenseDoc);
