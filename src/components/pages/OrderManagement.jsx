...
export default function OrderManagement({ showSnackbar }) {
    ...
    const { seqIdToName, idToName, staffOptions } = useStaff();

    const getStaffName = (sid) => seqIdToName[sid] || idToName[sid] || sid || '---';
    ...
    const fetchOrders = async (isLoadMore = false) => {
        ...
        try {
            ...
            let q = supabase
                .from('orders')
                .select('id, order_number, shift_id, status, subtotal, total, amount_tendered, change, payment_method, customer_id, customer_name, customer_phone, customer_address, customer_tin, staff_id, timestamp, invoice_status')
    ...
    const filteredOrders = useMemo(() => {
        return orders.filter(o => {
            ...
            // 2. Staff Filter
            const matchesStaff = staffFilter === 'ALL' || o.staff_id === staffFilter;
    ...
    const handlePrintReceipt = async (order) => {
        ...
        const data = normalizeReceiptData({ ...order, items }, {
            staffName: getStaffName(order.staff_id),
            isReprint: true
        });
    ...
    const handlePrintInvoice = async (order) => {
        ...
        const data = normalizeInvoiceData({ ...order, items }, {
            staffName: getStaffName(order.staff_id),
            isReprint: true
        });
        ...
        <Box sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
                        ...
                        {filteredOrders.length === 0 ? (
                            ...
                            filteredOrders.map((o) => (
                                <TableRow key={o.id} hover sx={{ opacity: o.status === 'VOIDED' ? 0.6 : 1 }}>
                                    ...
                                    <TableCell>{getStaffName(o.staff_id)}</TableCell>
                            ...
                            <Box sx={{ p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
                                <Stack spacing={1.5}>
                                    <Box display="flex" justifyContent="space-between">
                                        ...
                                        <Typography variant="body2" fontWeight="bold">{getStaffName(o.staff_id)}</Typography>
                                    </Box>
            ...
            {reprintOrder && (
                <SimpleReceipt
                    ...
                    staffName={getStaffName(reprintOrder.staff_id)}
                />
            )}
            {printInvoiceData && (
                <ServiceInvoice
