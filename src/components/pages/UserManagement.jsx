...
export default function UserManagement({ showSnackbar }) {
  ...
  const [infoDialog, setInfoDialog] = useState(null); // { title, message, password? }
  ...
  const handleResetPassword = async (u) => {
    try {
      ...
      const tempPassword = "Reset" + Math.random().toString(36).slice(-5).toUpperCase() + "!";
      ...
      // 3. Update the profile to set requires_password_reset flag
      ...
      setInfoDialog({
        ...
        message: `Password for ${u.full_name || u.email} has been reset. Please give this to the user. They will be forced to change it on login.`,
        password: tempPassword
      });
      showSnackbar?.(`Password reset for ${u.full_name || u.email} successfully.`, "success");
    } catch (err) {
  ...
  const handleOnboard = async (u) => {
    try {
      ...
      setInfoDialog({
        ...
        message: `User ${u.full_name || u.email} onboarded! Please give this to the user. They will be forced to change it on login.`,
        password: tempPassword
      });
  ...
  return (
    <Box sx={{ width: "100%", p: 3 }}>
                  ...
                  <TableRow key={u.id} hover sx={{ opacity: u.suspended ? 0.7 : 1 }}>
                    ...
                    <TableCell align="center">
                      <Stack direction="row" spacing={0.5} justifyContent="center">
                        ...
                        <Tooltip title="Reset password (creates temporary password)">
      ...
      {/* Info Dialog for Passwords and Alerts */}
      <Dialog open={!!infoDialog} onClose={() => setInfoDialog(null)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          {infoDialog?.password && (
            <Box sx={{ mt: 2, p: 2, bgcolor: "background.default", borderRadius: 1, textAlign: "center", border: '1px dashed' }}>
        ...
        <DialogActions>
          {infoDialog?.password && (
            <Button
              onClick={() => {
                navigator.clipboard.writeText(infoDialog.password);
          ...
          <Button onClick={() => setInfoDialog(null)} variant="contained" color="primary">
            OK
          </Button>
        </DialogActions>
      </Dialog>
