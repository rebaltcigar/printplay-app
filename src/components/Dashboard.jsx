...
// Combined array of all required denominations for easy validation
const allDenominations = [...denominationsMap.bills, ...denominationsMap.coins];
...
  const [denominationsError, setDenominationsError] = useState(false); // State for validation error display
...
    // Optionally clear error state immediately on input
    if (denominationsError) setDenominationsError(false); 
  };
 
  const handleConfirmEndShift = async () => {
    // Re-check validation before proceeding
    if (!isEndShiftFormValid) {
      setDenominationsError(true);
      alert("Please fill out the PC Rental Total and all cash denomination counts before ending the shift.");
      return;
    }
    
...
  const isEndShiftFormValid = useMemo(() => {
    const isPcRentalFilled = pcRental !== '';
    // Checks if all required denominations have an entry (not empty or undefined)
    const allDenominationsFilled = allDenominations.every(denom => denominations[denom] !== '' && denominations[denom] !== undefined);
    return isPcRentalFilled && allDenominationsFilled;
  }, [pcRental, denominations]);
...
          <TextField 
            autoFocus margin="dense" label="PC Rental Total" type="number" fullWidth variant="standard" 
            value={pcRental} onChange={(e) => setPcRental(e.target.value)} 
            required
            error={denominationsError && pcRental === ''}
            helperText={denominationsError && pcRental === '' ? "PC Rental Total is required." : ""}
          />
          <Divider sx={{ my: 2 }} />
          <Typography>Bills</Typography>
          {denominationsMap.bills.map(denom => (
            <TextField key={denom} margin="dense" label={`₱${denom} x`} type="number" size="small" 
              value={denominations[denom] || ''} 
              onChange={(e) => handleDenominationChange(denom, e.target.value)} 
              required
              error={denominationsError && (denominations[denom] === '' || denominations[denom] === undefined)}
              sx={{ width: '45%', mr: '5%', mb: 1 }} // Style for better layout
            />
          ))}
          <Divider sx={{ my: 2 }} />
          <Typography>Coins</Typography>
          {denominationsMap.coins.map(denom => (
            <TextField key={denom} margin="dense" label={`₱${denom} x`} type="number" size="small" 
              value={denominations[denom] || ''} 
              onChange={(e) => handleDenominationChange(denom, e.target.value)} 
              required
...
          {denominationsError && !isEndShiftFormValid && <Typography color="error" sx={{mt: 1}}>Please fill out all required fields marked with an error.</Typography>}
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseDialog}>Cancel</Button>
          <Button onClick={handleConfirmEndShift} variant="contained" disabled={!isEndShiftFormValid}>Confirm & End Shift</Button>
        </DialogActions>
      </Dialog>
