...
/* ---------- NEW UTC+8 FORMATTING HELPERS ---------- */
const toLocaleDateStringPHT = (ts) => {
  if (!ts?.seconds) return "";
  // Formats date like "10/20/2025" in UTC+8
  return new Date(ts.seconds * 1000).toLocaleDateString("en-US", { timeZone: "Asia/Manila" });
};

const toYMD_PHT_fromTS = (ts) => {
  if (!ts?.seconds) return "";
  // Formats a Firestore Timestamp into "YYYY-MM-DD" in UTC+8, for date inputs.
  // The 'en-CA' locale reliably produces this format.
  return new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Manila" }).format(new Date(ts.seconds * 1000));
};

const toLocalISO_PHT_fromTS = (ts) => {
  if (!ts?.seconds) return "";
  const d = new Date(ts.seconds * 1000);
  // Create a new Date object representing the time in PHT by adding the 8-hour offset.
  // This is safe because the Philippines does not observe Daylight Saving Time.
  const phtDate = new Date(d.getTime() + 8 * 60 * 60 * 1000);
  // Use toISOString() and slice to get the "YYYY-MM-DDTHH:MM" format required by datetime-local inputs.
  return phtDate.toISOString().slice(0, 16);
};

// Gets today's date as "YYYY-MM-DD" in UTC+8
const todayYMD_PHT = () => {
...
  if (label) return label;
  // Use UTC+8 for determining shift name
  const d = new Date(startTS.seconds * 1000);
  const h = parseInt(new Intl.DateTimeFormat('en-US', { hour: 'numeric', hour12: false, timeZone: 'Asia/Manila' }).format(d));
...
                    </TableCell>
                  </TableRow>
                  <TableRow><TableCell colSpan={9} sx={{ p:0, border: 0 }}><Divider /></TableCell></TableRow>
                </React.Fragment>
              ))}
            </TableBody>
          </Table>
...
                            <TableRow key={r.id}>
                                <TableCell>
                                    {toLocaleDateStringPHT(r.periodStart)} – {toLocaleDateStringPHT(r.periodEnd)}
                                </TableCell>
